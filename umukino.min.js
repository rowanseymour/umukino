/* Umukino :: Copyright Rowan Seymour 2011 */ var umu={ui:{}};function __extends(clazz,obj,params){clazz.apply(obj,params)}function __retain(fname,obj){eval("obj.super$"+fname+" = obj."+fname)};function Vector2(x,y){this.x=x;this.y=y;this.clone=function(){return new Vector2(this.x,this.y)};this.add=function(v){return new Vector2(this.x+v.x,this.y+v.y)};this.subtract=function(v){return new Vector2(this.x-v.x,this.y-v.y)};this.scale=function(s){return new Vector2(this.x*s,this.y*s)};this.dot=function(v){return this.x*v.x+this.y*v.y};this.length=function(){return Math.sqrt(this.x*this.x+this.y*this.y)};this.length2=function(){return this.x*this.x+this.y*this.y};this.equals=function(v){return this.x==
v.x&&this.y==v.y}};umu.Timer=function(host,callback,idleMs){this.host=host;this.callback=callback;this.idleMs=idleMs;this.timerId=0;eval("$timer_"+this.id+" = this;");this.start=function(){this._requestNextTick()};this.stop=function(){clearTimeout(this.timerId)};this._requestNextTick=function(){this.timerId=setTimeout("$timer_"+this.id+"._tick()",this.idleMs)};this._tick=function(){var time=(new Date).getTime();this.callback.apply(this.host,[time]);this._requestNextTick()}};umu.Resources=function(){this.imagePaths=new Array;this.audioPaths=new Array;this.count=0;this.loadedCount=0;var resources=this;this.addImage=function(key,src){this.imagePaths.push([key,src]);++this.count};this.addAudio=function(key,src){this.audioPaths.push([key,src]);++this.count};this.load=function(onLoaded,onProgress){this.loadedCount=0;this.onLoaded=onLoaded;this.onProgress=onProgress;this.image=new Object;this.audio=new Object;for(var r=0;r<this.imagePaths.length;++r){var key=this.imagePaths[r][0];
